...
def startNetwork():
    ...
    attacker = net.get('h3')
    targets = [net.get('h4'), net.get('h5'), net.get('h6')]

    print(f"Initiating Port Scan from {attacker.name}")


    for target in targets:
        target_ip = target.IP()
        print(f"Scanning target: {target.name} ({target_ip})")

        for port in range(20, 25):  
            attacker.cmd(f"hping3 -S -p {port} -c 1 -t 5 {target_ip}")
            sleep(0.5)  
